...
RDAP is more reliable than the `whois` binary and is safer for simple batch checks.
...
```bash
set +e +o pipefail   # disable strict error handling for batch probing if enabled
names=(foo bar baz)
...
- If the shell has `errexit` and/or `pipefail` enabled, start with `set +e +o pipefail` so one failed probe does not abort the batch.
- Do NOT use the `whois` binary in loops: it may throttle or fail after repeated queries, which can produce misleading results.
- `.co` and many ccTLDs have no public RDAP. Use socket whois where appropriate; avoid the `whois` binary for batches.
- Some RDAP services may reject clients with no User-Agent. Use `curl` as in the script.
- `/dev/tcp` may be unavailable in some shells; `nc` to port 43 may also be blocked. Prefer RDAP where available.
- If a registry returns a rate-limit code such as `?429`, slow down and recheck.
