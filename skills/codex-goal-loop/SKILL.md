...
description: Explain and write effective instructions for OpenAI Codex's `/goal` feature — the persistent self-checking agent loop (plan → act → test → review → iterate). Use when the user mentions Codex `/goal`, "goal loop", wants to kick off a long-running autonomous Codex run, asks how to write a goal prompt, or wants a one-paragraph goal instruction drafted.
---
...
`/goal` is a slash command in **Codex v0.128.0+** that turns a Codex prompt into a **persistent agent** looping `plan → act → test → review → iterate` until a stop condition is met, the user pauses, or the token budget runs out.
...
- **ChatGPT auth** — API-key auth does **not** work.
...
- **Never instruct the agent to create new ADRs** — ADRs require explicit human approval, so goal prompts must not pre-approve or encourage them.
...
- Keep approvals/sandboxing tight; default permissions are usually appropriate.
...
| Symptom | Fix |
|---|---|
| `/goal` missing from slash popup | Update Codex to a compatible version |
