---
name: cyber-audit
description: Read-only exposure audit of a local workstation and project directories for a CVE, breach, malicious package, or other security advisory, then write a structured report to a configured audit output directory. Use when the user shares a breach/CVE/malware/supply-chain advisory and asks if they are affected, says "scan my system for X", "are we affected by Y", "check if I'm vulnerable to Z", or requests any hack/breach/cyber/vulnerability audit on this workstation. Output should match the local audit report format.
...
- **Read-only.** No installs, removes, upgrades, restarts, network calls, or file modifications outside the configured audit output directory.
- **No `sudo`.** Never.
...
4. **Write the report** to `<audit-output-dir>/YYYY-MM-DD-<short-kebab-slug>.md`. Use today's date from the environment header.
...
ls /opt/homebrew/lib/node_modules 2>/dev/null                       # common macOS global npm path
...
find <project-roots> -maxdepth 6 \( -name "requirements*.txt" -o -name "pyproject.toml" \
...
# --- Env vars that change exposure, such as listening address configuration ---
...
File: `<audit-output-dir>/YYYY-MM-DD-<short-kebab-slug>.md`
...
Match the tone of existing reports in the configured audit output directory — terse, factual, bulleted, no hedging.
...
Existing reports in the configured audit output directory can be used as style references when present. Prefer the concise `YYYY-MM-DD-*.md` audit format over any longer baseline-audit format.
