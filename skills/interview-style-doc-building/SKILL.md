...
A mode for creating durable strategic docs. The AI does not propose content — it asks one question, the user answers, the AI patches the file, then asks the next question. The file is the conversation's output, updated incrementally.
...
- Quarterly or annual reviews where the user's words go into the file.

**NOT for:** day planning, task triage, or anything where the AI proposes content first.

## The Loop

1. **Create the file** with a skeleton: header, sections, and "to be filled in" placeholders. Use a single file creation/write operation for the new file. After this, never overwrite — only patch.
...
- **Lists from the user are unordered sets.** When the user lists items in response to "which X should we cover?" or "what are the Ys?", that is a set, not a ranking. Never infer rank, priority, or sequence from the order typed. If ordering is needed, ask explicitly: "Which of these is #1?"
- **Ask dynamics, not names.** When the user references a person, don't ask "who is X?" — ask about the role or dynamic.
...
- **Surface new reality.** Each question should pull out information the AI doesn't already have.
...
- **Assuming order from a set.** User lists "A, B, C, D" → AI writes "1. A, 2. B, 3. C, 4. D" → this is wrong. Always confirm rank explicitly.
- **Asking too many questions at once.** Even bundling two questions violates the rule.
- **Overwriting the file** instead of patching specific sections, which can destroy prior content.
- **Adding AI-generated content** to fill out sections. Sections stay empty until the user provides the content.
- **Skipping the file update** between Q&A pairs, causing the doc to fall out of sync.

## Pairing with Other Skills

- `day-plan` — different pattern for day planning or task triage.
- `organize-tasks` — separate workflow for organizing tasks.
- `memory-management` — separate workflow; durable preferences or persona details should go to memory.
