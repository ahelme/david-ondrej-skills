...
The `pi-web-access` package provides web access. It supports zero-config search where available, with provider fallbacks.

## CRITICAL: always pass `workflow: "none"`

Every `web_search` call MUST include `workflow: "none"`. This skips the interactive browser curator popup. No exceptions — single query or batched `queries`, always set `workflow: "none"`.
...
- **GitHub URLs are cloned, not scraped** — you get real files plus a local checkout to explore with standard file-reading tools.
- **PDFs** → auto-extracted to markdown, readable in sections where supported. Text-only; no OCR.
- **YouTube/video** → transcripts and, where configured, frame extraction. Some providers or media tooling may require local configuration.
...
- **"deep research"** → **at least 8** queries, totally different keywords and angles, run across 2–3 successive batches and refine angles after each batch.
