...
Ensure an OpenRouter API key is available in the environment as `OPENROUTER_API_KEY`. If it is not set, configure it using your normal secrets-management workflow. Never invent or hard-code a key.
...
- Submit results for the official leaderboard using the public submission instructions from the benchmark maintainers.
